...
//                        convertErrorBody(response.errorBody())
                    ...
                    Log.v("MyTag", response.message().toString())
                    NetworkResult.Error(
                        message = response?.body().toString() ?: "Something went wrong"
                    )
                }
...
//    private fun convertErrorBody(errorBody: ErrorResponse?): ErrorResponse? {
//        return try {
